...
class AdminController extends Controller
{
        ...
        public function adminInfo($id)
    {
        ...
        return view('dashboard.admin-info', compact('admins'));
...
$team = Team::where('manager_email', $request->email)->get();
...
} else if (in_array(2, $request->roles) || in_array(3, $request->roles)) {

            return redirect()->back()->with('error', 'لا يمكن إضافة صلاحية مدير فريق لهذا العضو فهو لا يمتلك فرق مسجلة باسمه');
...
if (in_array(2, $request->roles) || in_array(3, $request->roles)) {
       ...
       return redirect()->back()->with('error', 'لا يمكن إضافة صلاحية مدير فريق أو بطولة لهذا العضو');
